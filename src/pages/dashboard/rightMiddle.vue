  ...
  <div class="BlueHorn">
    ...
    <div class="item">
      <div ref="chart"></div>
        ...
        <!--点击弹窗-->
		<point-dialog :right="true" :name="this.result.mlist" :echartData="echartData" :detialData="detialData" v-if="mesFalse2" :dialogStyle="dialogStyle" @close="mesFalse2 = false">
		</point-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
      ...mapGetters(['QuotaList', 'SeQuota','screenType', 'dimen', 'spaceCheckId', 'timeTypeId', 'networkType', 'cityId', 'time', 'indexIds','spaceType']),
      dataRange () {
        const { time, timeTypeId, indexIds, QuotaList, SeQuota,spaceType,cityId } = this
        return { time, timeTypeId, indexIds, QuotaList, SeQuota,spaceType,cityId }
      }
    },
  watch: {
    dataRange(val) {
  ...
  methods: {
    initData () {
      if (this.isSignle) {
        ...
        chartApi.getSceneAnalyse(params).then(res => {
        ...
        chartApi.getSceneAnalyse2(params).then(res => {
          if (res.code === 200) {
            this.result = res.result
            this.initChart()
          } else {
    ...
    initChart () {
      try {
        ...
        if (num == 1) {
          ...
          this.option.yAxis.name = this.result.mlist[0].pname.replace(/(.{8})/g,'$1\n')
        } else if (num == 2) {
        ...
        this.chart.on('click', function (params) {
          ...
          chartApi.getSceneAnalyCellList(data).then((res) => {
            ...
            _this.dialogStyle = {
              ...
              left: `${offsetLeft + 10 -220}px` // 往左偏移 110
            }
          })
        })
      } catch (e) {
